...
\begin{itemize}
    \item a verdict location, which means the trace conforms to
        ...
        $D(\EuScript{S}^N)$). The verdict is $Pass$;
...
\begin{algorithm}[h]
    ...
    \While{valued event $(a(p), \alpha)$}{
        ...
        \For{$i = 1, \dots, n$}{
                ...
                \If{$(r, col) == (q_0, 0)$}{
                    ...
                    with $G = M_{[b]}[j, *]$}{
                        ...
                        \For{$c_{[b]} = 1, \dots, k$}{
                            ...
                            \If{$\alpha \cup v \models M_{[b]}[j, c_{[b]}]$}{
            ...
            %
            % TODO: Attention, ne fonctionne pas avec un modèle
            % qui ne gère pas la première transition, exemple :
            % l0 -- b --> l1 et on envoie `a`
            %
...
%            }
        ...
        \If{$\forall ~i \in \{ 1, \dots, n \}, ~RUNS_i == \emptyset$}{
            return $\{ trace \}$\;
        }
...
limit. Another approach would be to compute average delays
between the events in the models $R(\EuScript{S}_i)$ thanks to
the event's time stamps.
