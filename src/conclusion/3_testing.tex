...
manufactured, and one for each model set ($R(\EuScript{S}^N)$ and
$D(\EuScript{S}^N)$). The intuition of such an algorithm is to
wait for incoming valued events sent by the entry point, and to
construct a trace until it reaches:
...
\ref{algo:online-entry-point}). % TODO: fixme
...
$r$, for all $(r, col) \in RUNS_i$ (line 25). % TODO: fixme
If $l$ is not a \emph{verdict location} (cf.
\crossref{sec:testing}{sec:testing:normal}), the trace does not
            ...
            \ForEach{$(r, col) \in RUNS_i \mid r = q_0 (a_1(p), \alpha_1) \dots q_{j - 1}$
                    ...
                    with $G = M_{[b]}[j, *]$}{\label{algo:check-online:line:transition}
                        ...
                        \For{$c_{[b]} = 1, \dots, k$}{
                            ...
                            \If{$\alpha \cup v \models M_{[b]}[j, c_{[b]}]$}{\label{algo:check-online:line:sat}
                                ...
                                $RUNS' = RUNS' \cup \{ (r \cdot (a(p), \alpha) \cdot q_{next}, c_{[b]}) \}$\;
                            }% endif sat M
                ...
                \Else{
                    ...
                    with $G = M_{[b]}[j, col]$}{
                        ...
                        \If{$\alpha \cup v \models M_{[b]}[j, col]$}{
