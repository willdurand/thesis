...
% TODO: use this paragraph somewhere
% Both offline and online modes are not completely unalike, they
% also serve different purposes. Our online passive testing
% approach records traces on a system under test on-the-fly, and
% then check whether those traces satisfy specifications, still
% generated from a system under analysis. It enables what we call
% \emph{just-in-time fault detection}. Faults can be revealed in
% near real-time on a running system so that users can be notified
% as soon as possible.
