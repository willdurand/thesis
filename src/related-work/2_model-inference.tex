...
collect positive or negative observations. However, this can lead
to some issues like disturbing the system for instance. This is
not always suitable, and that is why another category of methods
infer models in a passive manner, i.e. without stimulating the
software system.  In the next section, we introduce some of these
passive model inference techniques.
...
Acceptor} (PTA), which is a tree whose edges are labelled with
...
merges more states than kTail. However, the resulting FSA
...
transitions are still labelled with an event and a symbol
...
calls. This produces a large set of traces. Assuming that methods
...
describing method calls of API from source code of the client
...
the transitions, labelled with operation names and input/ouput
