...
Most of the following approaches, which build models from execution traces bu means of event sequence abstraction, stand on two algorithms: \textit{ktail} \cite{5009015} and \textit{kbehavior} \cite{mariani2007dynamic}.
...
%%weaker than bissimilation minimisation
%
...
%%%%%over -under approximations weaker than bisimulation
...
automatically.
%Another more general and formal solution to interface synthesis is given in
...
where each group represents a usage scenario for a given class.%runtime
...
upon concolic testing to explore symbolic execution paths of applications and to detect bugs. In \cite{concolicandroid12},
...
%limitation,
%instrumentation donne bq de traces, exploration of source code, long These white-box approaches theoretically offer a better
%code coverage than black-box automatic testing. However, the
%number of paths being explored concretely limits to short paths
%only, and the constraints have not to be too complex for being
%solved.
