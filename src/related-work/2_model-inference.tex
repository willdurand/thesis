...
techniques in Section \vref{sec:active-letoile}, incremental
learning techniques in Section \vref{sec:active-increment}, and
the model generation by means of automatic testing of
event-driven applications providing Graphical User Interfaces
GUIs) in Section \vref{sec:active-crawling}. Passive inference
...
\vref{sec:passive-fsa}, using state-based abstraction in Section
\vref{sec:passive-spec}. Then, we present some white-box
approaches in Section \vref{sec:passive-white}, and we also
mention a few techniques that leverage documentation to infer
models in Section \vref{sec:passive-others}.
...
\cite{Raffelt:2005:LLA:1081180.1081189,irfan12}, others proposed
to apply it in specific application contexts, as summarized
below.
\TODO{add more refs}
...
\begin{table}
	...
	informal models (IM). Column 6 gives the strategy or
	algorithm used to crawl the application. Last column (7)
	shows whether the technique handles potential crashes, \emph{i.e.}
...
methods infer models in a passive manner, \emph{i.e.} without
