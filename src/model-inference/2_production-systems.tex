\chapter{Inferring models for production systems}
...
%Until now, this huge amount of
%data was not leveraged, however a quick analysis shew us that a
%lot of really interesting information could be extracted.
...
As stated in the framework overview, production events are
...
We obtain a set $ST=\{ST_1,...,ST_N\}$ according to the following
proposition:
		...
		\fbox{%
			\begin{minipage}{0.46\textwidth}
				\begin{tabular}{l}
					...
					{(l_0,v_\emptyset) (a_1,\alpha_1) (l_{k1},v_\emptyset) ...(l_{kn-1},v_\emptyset) (a_n,\alpha_n) (l_{kn},v_\emptyset) \in Runs_i}$
				\end{tabular}
				\newline
			\end{minipage}%
		}
...
\begin{definition}
	...
	\noindent\fbox{%
		\begin{minipage}{0.47\textwidth}
			\begin{center}
				\begin{tabular}{l}
					$\frac{(l_i,v_\emptyset) (a_i,\alpha_i) (l_{i+1},v_\emptyset) \in r, p=\{x \mid (x:=v)\in \alpha_i \}, G_i=\displaystyle \bigwedge_{(x:=v)\in \alpha_i}x==v}{l_i
                        \xrightarrow{a_i(p),G_i,id_V}_{\EuScript{S}_i} l_{i+1}}$\\
				\end{tabular}
			\end{center}
		\end{minipage}%
	}
...
%efficient manner. That is why we worked on reducing it.

\subsection{STS reduction}
\label{part3:reduction}
%Our first STS has a tree form where branches are directly
%derived from traces.
...
To explain how \textit{Autofunk} works, we consider a case study
...
%Furthermore, we observed that the STSs may
...
% TODO: ******STS higher level of abstraction******
%
%******complexité*****************
%
%Our algorithm has a time complexity in $O(n)$ where $n$ is the
%number of actions. Then, each trace is sorted to retrieve the
%initial sequence with actions in the right order. In our
%implementation, we use the Java $Collection.sort()$ method for
%sorting purpose. Its complexity is $O(n * log(n))$ according to
%the Java documentation, $n$ being the number of actions in a
%structured trace ($n << number of all actions$). To summurize,
%the complexity for transforming the remaining actions of the
%knowledge base into a structured trace set is $O(m * n * log(n))$
%with $m$ the number of structured traces.
%
%*******
