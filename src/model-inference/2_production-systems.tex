...
\begin{itemize}
    \item Most of the existing model inference approaches give
        ...
        be used for regression testing, and fault diagnosis;
...
$Rinit((point:=val1) \in \alpha_1)++$\;
$Rfinal((point:=val2) \in \alpha_n)++$\;
}
...
\For{$i = 1, \dots, n$}{
    \ForEach{$\alpha_i=(point:=val) \in POINT_{init}$} {
        ...
        \alpha_i \in \alpha_1, \exists ~(point:=val2) \in
...
minutes. Therefore, \textit{Autofunk} can be used to quickly infer
...
which is neither smart nor accurate. That is why we chose to work
on a new solution to segment and filter traces, which gave birth
to \emph{Autofunk v3}.
