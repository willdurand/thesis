...
\TODO{exact: link to def "as defined in ..."}
...
\begin{itemize}
    ...
    systems is complicated since these must not be disrupted
    (otherwise it may lead to severe damages on the production
     machines),
...
to come up with a better version of our \textit{Autofunk}
framework that is presented in this thesis.
...
\textit{INFO}, \textit{7011}, and \textit{17021} are labels along
...
of traces denoted by $Traces(Sua)$. Figure \ref{fig:tsua}
...
This module performs these two steps, which are summarised in
...
\TODO{complexité, sound, complete?}
...
\TODO{explain constraints: vérifier que STS v0 => condition...WAT?}
...
\TODO{impl. with smart algo based on hashes}
...
depicted in Figure \ref{fig:autofunk_branded}. It shows how we
...
RabbitMQ\footnote{https://www.rabbitmq.com/}). Initial benchmarks
...
architecture presented in Figure \ref{fig:autofunk_branded}.
...
to generate models from production systems. Given a large set of
production events, our framework infers exact models whose traces
are included in the initial trace set of a system under analysis.
