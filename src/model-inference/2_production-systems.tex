...
GUI applications, e.g., desktop or mobile applications. Yet, we
...
\ref{sec:modelinf:prodsystems:autofunk}, we present \textit{Autofunk} for
...
contains different modules (in gray in the figure): four modules
...
\begin{figure}[ht]
    ...
    modules in Gray along with their corresponding steps. The last
...
time-stamped, has a label (e.g., $17011$), and may own
...
\begin{figure}[ht]
    ...
    \caption{The architecture of \textit{Autofunk} that has been used to
...
a signature for each branch $b$, i.e. a hash
...
plan to integrate \textit{Autofunk} with any Michelin production system.
Events are collected on the fly from a system under analysis, and
sent to a database
...
system. \textit{Autofunk} then queries the database to fetch product
...
\begin{figure}[ht]
    ...
    \textit{Autofunk} in a production environment.}
