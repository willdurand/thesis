...
\begin{enumerate}
    ...
    \item \textbf{No implicit knowledge elimination:} after the
        ...
        transition base, for a transition $t =
        (l_n,l_m,a(p),G,A)$ extracted from $T_{i+1}$, $l_n$ is
        still reachable from $l0$, \emph{i.e.} rules should not
        break the existing IOSTS paths.
\end{enumerate}
...
\emph{e.g.}, using a machine learning technique, but we did not
pursue this path.
% TODO: is it in perspectives?
