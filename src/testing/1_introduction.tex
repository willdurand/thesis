...
relation corresponds to the trace preorder \cite{DNH84}, which is
...
behaviors that should happen. That is why we propose a second
...
In Section \ref{sec:testing:normal}, we introduce a new step of
...
inferred with \textit{Autofunk} that we \textit{normalize} to get rid of
...
normalize these models before using them for testing.  The
...
behaviors that should happen}. These behaviors are represented
...
extension. While the previous \textit{Autofunk}'s architecture has been
kept, there is two new modules: STS Normalization and "check",
...
In online mode, we do not have traces but rather filtered valued
events of $\mathit{Sut}$ coming on-the-fly. Finally, we perform
...
The first implementation relation, denoted by $\leq_{ct}$,
